...
## 2026-09-24 (evening) — Search budget: deterministic A* cap, anytime `-t` kept, opt-in incumbent

User's goal: more seeds, more time or more iterations must never give a
*worse* volume, while keeping MCTS's anytime property (the 2 s `-t`).

### What was measured

`TOPOLS_ASTAR_STATS` (env-gated, `routing/astar.py`) logged every A* call
on bv_16 / dj_16 / qaoa_16 at production config (job 4812, 2.5M calls):

| outcome | expansions (median / p99 / max) | seconds |
|---|---|---|
| success (`ok`) | 2 / 47 / **303** | < 1 ms typical |
| no path (`exhausted`) | 1 / 365 / 588 | < 6 ms |
| wall-clock `timeout` (0.1 s) | 16.5k / 19.5k / 22k | 0.100 s |

So the 0.1 s route timeout (~16-19k expansions on this machine) only ever
ended hopeless searches, 50x beyond any real route -- but it did so
*non-deterministically*: under load a route that needed 300 expansions
could still be cut, changing one layer's embedding and which fallback
tier fired. **Replaced by a deterministic cap**: `MAX_EXPANSIONS = 5000`
(`shortest_path_with_zmax` / `shortest_path`), `MAX_EXPANSIONS_BASE =
500` (`shortest_path_base`, old 1 ms ~ 165 expansions). >16x any observed
route, 3-4x sooner than the timeout on hopeless ones. `timeout` stays in
the signatures, unused.

### Tried and abandoned: a pure iteration budget

First attempt made `-t` a 600 s safety cap and `-i` the only budget, for
full determinism. bv_16 at `-i 1000` gave the same 486 / 21-21 but took
**559 s instead of 13 s**; `-i 150` still 88 s. Reason (already in the
2026-09-22 boundedness profiling): the 2 s wall clock gave every layer the
same *compute*, letting cheap layers finish 1000 iterations and expensive
ones only ~12-34% -- a fixed `-i` forces the expensive layers to the same
count and time explodes, while a small `-i` makes layers fail into the
fallback ladder (job 4473 lesson). Reverted; the user wants the anytime
knob. Jobs 4813/4814 cancelled; `exp.py` / regression configs / full-suite
script back to `-t 2`; `prog.py` default 3.

### What holds now, and what cannot

Within one `mcts()` call the iteration sequence is a deterministic
function of (seed, state, N): `moves()`/`rollout()` draw only from the
seeded RNG and routing is now deterministic, and the call returns the
best rollout seen. So **more iterations, or more wall-clock time (a
larger N), can only extend the same sequence -- never worse for that
call**; more seeds is an argmax over a superset -- never worse for that
layer. What remains non-deterministic is only how many iterations fit in
2 s, and the per-layer greedy choice is still not globally monotone.

For a *strict* cross-run guarantee: `prog.py --incumbent 1` (default 0)
keeps the existing `result/topols/<name>.pkl` when its stored `volume` is
<= the new run's, prints "Incumbent kept", and records `incumbent_kept`
in the CSV. With it on, re-running with more `-t`/`-s`/`-i` is monotone
in volume by construction. Off for regression runs so they always report
the current code.

Verification of the cap alone at production config (job 4815): bv_16
486 / 21-21, dj_16 648 / 31-31, qaoa_16 4860 / 48-48, all PASSED (qaoa_16
has now been seen at 4698 / 4860 / 5022 across runs -- the 2 s wall
clock's iteration-count noise, which `--incumbent 1` is for).
